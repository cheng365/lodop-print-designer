...
<script>
import { getCodeTypeArray, getItemTypeArray, getLineStyle } from '../../libs/props.js'
...
export default {
  ...
  methods: {
    handleLineTypeChange(val) {
      const { type } = this.lineStyleArray.find(item => item.value === val)
      this.activeElement.style.type = type
    }
  },
}
</script>
